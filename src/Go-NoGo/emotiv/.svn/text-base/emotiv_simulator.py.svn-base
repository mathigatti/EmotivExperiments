...
class Emotiv:
    ...
    def __acquire(self):
        ...
        while self.stopflag == False:
            nSamplesTaken = random.randint(0,100)
            if nSamplesTaken != 0:
                self.channelsLock.acquire()
                for i, val in enumerate(self.channels):
                    xs = numpy.arange(1, nSamplesTaken, .01)[0:nSamplesTaken]
                    signal = numpy.sin(xs * .3)
                    noise = numpy.sin(xs) + \
                            numpy.sin(xs*2.33)*.333 + \
                            numpy.sin(xs*2.77)*.777
                    static = (numpy.random.random_sample((len(xs))) - .5) * .2 
                    self.data[i] = (signal + noise + static).tolist()
                # Random marks
                self.data[23] = [0] * nSamplesTaken

                marcas = (6, 10, 14, 18, 22, 26, 30, 34, 38, 42, 46, 50)
                # if random.randint(0, 1):
                if True:
                    if not self.letterStarted:
                        self.lettersFlashed = 0
                        self.letterStarted = True
                        pos = random.randint(0, nSamplesTaken - 1)
                        self.data[23][pos] = 3
                    elif self.lettersFlashed >= self.flashesPerTrial:
                        self.letterStarted = False
                        pos = random.randint(0, nSamplesTaken - 1)
                        self.data[23][pos] = 5
                    else:
                        pos = random.randint(0, nSamplesTaken - 1)
                        marca = marcas[random.randint(0, 11)]
                        self.data[23][pos] = marca
                        self.lettersFlashed += 1
